...
- **Bendu Olo**: The name used in this mod for the fallback NPC Champion of Cyrodiil. The Champion of Cyrodiil has no canonical personal name in lore; Bethesda's Oblivion Construction Set uses "Bendu Olo" as its developer-default character name, borrowed from the historical First Era Colovian Baron-Admiral (King of Anvil, commander of the All Flags Navy, ~1E 2260). This mod adopts that convention: "Bendu Olo" is the personal name given to the AI-controlled NPC spawned when all HoK candidates die before the Shivering Isles content fires, ensuring the Sheogorath succession always has a canonical vessel. He is Breton, matching the Oblivion Construction Set developer-default character race. The lore titles "Hero of Kvatch" and "Champion of Cyrodiil" are used as titles in flavor text only, never as a personal name.
...
2. If none exist (all HoK candidates dead before Shivering Isles fires) → spawn a new AI character: **Breton** (matching the Oblivion CS developer-default), named Bendu Olo, with `shivering_isles_champion` trait. This is the fallback vessel to ensure the Sheogorath succession always completes canonically.
